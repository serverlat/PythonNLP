...
def Task4a():
    ...
    print("The probability of a Noun(PP) being 'I' is",
          tagwords["PP"].prob("I"))
    #a
    print("The probability of a Noun(NN) being 'we' is", tagwords["NN"].prob("we"))
    #b
...
def Task4b():
    tagwords, tags = probDist(brown)
    taglist = ["START","PP", "VB", "PP", "NN"]
    words = ["I", "conduct", "my", "conduct"]
    result = 1
    for i in range(4):
        result *= tags[taglist[i]].prob(taglist[i+1])
        result *= tagwords[taglist[i+1]].prob(words[i])
    result *= tags["NN"].prob("END")
    print(f"The probability of 'PP VB PP NN' for 'I conduct my conduct' is {result}")
...
def Task4c():
    tagwords, tags = probDist(brown)
    distinct_tags = tags.keys()
    sentence = ["You", "should", "invest", "in", "the", "stock", "marked"]
    ViterbiBestTag(distinct_tags, tagwords, tags, sentence)
